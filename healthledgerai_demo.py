...
Version 1.0 | Maria Polychroniadou | healthledgerai.com
...
TODAY        = date(2026, 8, 1)   # reference date used in the dataset
...
def detect_phantom_billing(inv, appt):
    ...
    appt_dates = set(zip(appt["patient_id"], appt["date"].astype(str)))
    alerts = []
    for _, r in inv.iterrows():
        inv_date = str(r["date"])[:10]
...
def detect_unbundling(inv, bundles):
    """D5 — Bundle component codes billed individually on same day for same patient."""
    alerts = []
    for _, b in bundles.iterrows():
        bundle_name = b["bundle"]
        members = [c.strip() for c in str(b["member_codes"]).split(",")]
        patient_day = inv.groupby(["patient_id", "date"])["procedure"].apply(list).reset_index()
...
def detect_unregistered_doctor(inv, nets):
    ...
    for _, r in inv.iterrows():
        if pd.isna(r["insurer"]) or pd.isna(r["doctor_id"]):
            continue
        registered = network.get(r["insurer"], [])
        if r["doctor_id"] not in registered:
            alerts.append({
...
def detect_identity_collision(inv, pat):
    ...
    pat["full_key"] = pat["first"] + "|" + pat["last"] + "|" + pat["dob"].astype(str)
    ...
    if not dupes.empty:
        for key, group in dupes.groupby("full_key"):
            ...
            alerts.append({
                ...
                "risk": "HIGH",
                "invoice_id": ", ".join(affected_invoices[:5]) + ("…" if len(affected_invoices)>5 else ""),
                "patient_id": " / ".join(ids),
        ...
        if r["payment_method"] in ("cash", "card") and not pd.isna(r["insurer"]):
            ...
            if r["doctor_id"] not in registered:
                alerts.append({
                    ...
                    "invoice_id": r["id"],
                    "patient_id": r["patient_id"],
                    "detail": (f"Patient billed {r['payment_method'].upper()} despite having {r['insurer']} — "
                               f"doctor {r['doctor_id']} out-of-network"),
...
def detect_procedure_date_mismatch(inv, appt):
    ...
    appt_map = appt.groupby("patient_id")["date"].apply(lambda x: set(x.astype(str))).to_dict()
    alerts = []
    for _, r in inv.iterrows():
        inv_date = str(r["date"])[:10]
        appt_date = str(r["appointment_date"])[:10] if not pd.isna(r["appointment_date"]) else None
        if appt_date and inv_date != appt_date:
            alerts.append({
...
def print_summary(all_alerts):
    ...
    print("─"*65)
    for det, count in sorted(counts.items()):
        risk = next((a["risk"] for a in all_alerts if a["detector"]==det), "")
        print(f"  {RISK_COLOUR[risk]}  {det:<40}  {count:>3} alert(s)")
    print("═"*65 + "\n")
...
def main():
    ...
    for name, alerts in detectors:
        status = f"  ✓  {name:<35} {len(alerts):>3} alert(s)"
        print(status)
